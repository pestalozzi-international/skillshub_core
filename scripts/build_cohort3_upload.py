...
FINAL_SEL_PATH = sys.argv[1] if len(sys.argv) > 1 else str(
    REPO_ROOT / "data" / "FINAL_SELECTION.csv"
)
APP_FORM_PATH = sys.argv[2] if len(sys.argv) > 2 else str(
    REPO_ROOT / "data" / "The_Edulutmation.csv"
)
OUT_PATH = sys.argv[3] if len(sys.argv) > 3 else str(
    REPO_ROOT / "outputs" / "Cohort3_Student_Upload.xlsx"
)
...
GRADE_MAP = {
    ...
    "grade 12": "Grade 12", "grade 13": "Grade 12",
}
...
LEVEL_MAP = {
    "grade 1": "Grades 1 to 7", "grade 2": "Grades 1 to 7",
    "grade 3": "Grades 1 to 7", "grade 4": "Grades 1 to 7",
    "grade 5": "Grades 1 to 7", "grade 6": "Grades 1 to 7",
    "grade 7": "Grades 1 to 7",
    "grade 8": "Grades 8 to 9", "grade 9": "Grades 8 to 9",
    "grade 10": "Grades 10 to 12", "grade 11": "Grades 10 to 12",
    "grade 12": "Grades 10 to 12", "grade 13": "Grades 10 to 12",
}
...
for row in rows[2:]:
    ...
    final_students.append({
        "full_name": g(0), "first_col": g(1), "gender": g(3),
        "area": g(4), "last_grade": g(5), "year_left": g(6),
        "employed": g(7), "is_parent": g(8), "trainee_num": g(9),
        "sel_pre_score": g(11), "numeracy": g(13), "literacy": g(14),
        "nf": norm(g(0)), "nfirst": norm(fname(g(0))),
    })
...
MATCH_FILLS = {
    "exact": G_FILL, "first+area": G_FILL, "first+area(last)": Y_FILL,
    "first+grade": Y_FILL, "first_only": Y_FILL, "no_match": R_FILL,
}
...
col_widths = [
    14, 14, 18, 26, 8, 13, 16, 26, 16, 10, 8, 28, 14, 16, 16,
    12, 8, 10, 12, 8, 8, 28, 8, 12, 30, 22, 20, 20, 30, 30,
    20, 20, 14, 14, 14, 13, 10, 10,
]
...
ref_widths = [26, 12, 18, 14, 18, 8, 13, 16, 26, 14, 28, 14, 12, 28, 8, 8, 8, 10, 20, 30, 30, 10, 10, 10, 12, 8]
...
instructions = [
    ...
    ("Final Selection CSV: Master list of selected students (name, area, grade, scores)", False, 10, "000000"),
    ("Cohort 3 App Form: Full application data (DOB, phone, email, marital, employment, etc.)", False, 10, "000000"),
    ("", False, 10, "000000"),
    ...
    ("  - how_skill_benefits_community / how_skill_improves_livelihood: not in app form", False, 10, "000000"),
    ...
    ("  Pilot Cohort (was Cohort 1), Cohort 1 (was 2), Cohort 2 (was 3), Cohort 3 (was 4)", False, 10, "000000"),
    ("Ensure the cohort rename is complete in the portal before importing.", False, 10, "000000"),
    ("", False, 10, "000000"),
    (f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}  |  Total students: {len(rows_out)}", False, 9, "666666"),
]
