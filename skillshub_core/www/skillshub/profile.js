...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Profile CRUD — PUT to Frappe REST
  document.getElementById('contact-form').addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.exception || errData.message || 'Update failed');
      }
...
function fetchStudentSummary() {
  ...
  .catch(() => {
    document.getElementById('timeline-container').innerHTML =
      '<p style="color:var(--color-slate-500);">Error loading profile. Please refresh.</p>';
  });
}
...
function renderTimeline(enrolments, student) {
  ...
  if (!enrolments || enrolments.length === 0) {
    container.innerHTML = `<div class="timeline-item">
      <div class="timeline-title">Welcome to SkillsHub</div>
      <div class="timeline-description">You are enrolled in ${
        student.skillshub_programme || 'the programme'
      }. Your milestones will appear here.</div>
    </div>`;
    return;
  }
  container.innerHTML = enrolments.map(e => {
    const attRate  = e.attendance_rate != null
      ? ` · ${Math.round(e.attendance_rate)}% attendance` : '';
    const feedback = e.feedback_submitted
      ? ' · <span style="color:var(--color-emerald-700)">✓ Feedback</span>' : '';
    const baseline = e.baseline_submitted
      ? ' · <span style="color:var(--color-emerald-700)">✓ Baseline</span>' : '';
    return `<div class="timeline-item">
      <div class="timeline-date">${formatDate(e.enrolment_date)}${e.completion_date
        ? ' → ' + formatDate(e.completion_date) : ''}</div>
      <div class="timeline-title">${e.milestone || 'Programme'}${e.course
        ? ' · ' + e.course : ''}</div>
