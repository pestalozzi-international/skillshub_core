...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.querySelectorAll('.path-btn').forEach(btn=>btn.addEventListener('click',function(){
...
function loadData(){
  Promise.all([
    ...
    sf('/api/resource/SH Student Enrolment?fields=["student","milestone","feedback_submitted","attendance_rate"]&limit=1000').then(d=>{ allEnrolments=d.data||[]; })
  ])
  .then(()=>{
  ...
  .catch(()=>{ document.getElementById('content').innerHTML='<div class="state-box" style="color:var(--color-red-700)">Error loading data. Please refresh.</div>'; });
}
...
function renderTable(students){
  ...
  let html='<div class="table-wrap"><table><thead><tr><th>Student ID</th><th>Full Name</th><th>Cohort</th><th>Programme</th><th>Path</th><th>Attendance</th><th>Feedback</th><th></th></tr></thead><tbody>';
  students.forEach(s=>{
    const enrols=allEnrolments.filter(e=>e.student===s.name);
    const avgAtt=enrols.length ? Math.round(enrols.reduce((sum,e)=>sum+(e.attendance_rate||0),0)/enrols.length) : 0;
    const attClass=avgAtt>=80?'att-high':avgAtt>=60?'att-med':'att-low';
