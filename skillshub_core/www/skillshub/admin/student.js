...
if (!studentId) window.location.replace('/skillshub/admin/students');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  .then(function (results) {
    var sRes = results[0];
    var eRes = results[1];
    if (!sRes || !eRes) return; // clearAndRedirect already called
    render(sRes.data, eRes.data || []);
  })
  .catch(function () {
...
function render(s, enrolments) {
  ...
  var MILESTONE_ORDER = ['Mindset Camp', 'Soft Skills', 'Edulution', 'Vocational Training', 'Attachment'];
  var enrolMap = {};
  enrolments.forEach(function (e) { enrolMap[e.milestone] = e; });
  var skipEd = (s.programme_path === 'Path B');

  var timelineItems = MILESTONE_ORDER
    .filter(function (m) { return !(skipEd && m === 'Edulution'); })
    .map(function (m) {
      var e = enrolMap[m];
      if (!e) {
        return '<div class="tl-item tl-future"><div class="tl-dot tl-dot-future"></div>' +
          '<div class="tl-body"><div class="tl-title">' + m + '</div>' +
          '<div class="tl-meta">Not yet started</div></div></div>';
      }
      var att       = e.attendance_rate != null ? Math.round(e.attendance_rate) : null;
      var statusCls = e.status === 'Completed' ? 'tl-dot-done' : e.status === 'Dropped' ? 'tl-dot-drop' : 'tl-dot-active';
      var pills = '<span class="pill pill-status pill-' + (e.status || '').toLowerCase() + '">' + (e.status || 'Enrolled') + '</span>';
      ...
      return '<div class="tl-item"><div class="tl-dot ' + statusCls + '"></div>' +
        '<div class="tl-body">' +
  ...
  var empHtml = emp.length === 0
    ...
    : '<table class="emp-table"><thead><tr><th>Employer</th><th>Role</th><th>Type</th><th>Start</th><th>End</th></tr></thead><tbody>' +
      emp.map(function (r) {
        return '<tr><td>' + (r.employer_name || r.institution || '—') + '</td>' +
