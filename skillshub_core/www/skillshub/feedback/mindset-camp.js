  ...
  try {
    ...
    try {
      ...
      (aspects.data || []).forEach(a => {
        ...
        d.innerHTML = `<input type="checkbox" id="asp-${a.name}" value="${a.name}" class="aspect-cb">
                       <label for="asp-${a.name}">${a.name}</label>`;
        group.appendChild(d);
      });
      if (!aspects.data || aspects.data.length === 0)
        group.innerHTML = '<p style="color:var(--color-slate-500)">No aspects configured.</p>';
    } catch {
      document.getElementById('aspects-group').innerHTML = '<p style="color:var(--color-slate-500)">Could not load aspects.</p>';
    }
