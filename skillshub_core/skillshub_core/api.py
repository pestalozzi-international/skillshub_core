	...
	forms = [
		{
			...
			"route": FEEDBACK_ROUTE_MAP["SH Baseline"],
		},
		{
			"doctype": "SH Soft Skills Feedback",
			"label": "Soft Skills Feedback",
			"route": FEEDBACK_ROUTE_MAP["SH Soft Skills Feedback"],
		},
		{
			"doctype": "SH Mindset Camp Feedback",
			"label": "Mindset Camp Feedback",
			"route": FEEDBACK_ROUTE_MAP["SH Mindset Camp Feedback"],
		},
		{
			"doctype": "SH VT Feedback",
			"label": "Vocational Training Feedback",
			"route": FEEDBACK_ROUTE_MAP["SH VT Feedback"],
		},
		{
			"doctype": "SH Attachment Feedback",
			"label": "Attachment Feedback",
			"route": FEEDBACK_ROUTE_MAP["SH Attachment Feedback"],
		},
		{
			"doctype": "SH Parent Feedback",
			"label": "Parent Feedback",
			"route": FEEDBACK_ROUTE_MAP["SH Parent Feedback"],
		},
	]
	if path == "Path A" or not path:
		forms.insert(
			3,
			{
				...
				"route": FEEDBACK_ROUTE_MAP["SH Edulution Feedback"],
			},
		)
	return forms
...
def get_student_summary(student):
	...
	if _doctype_exists("SH Employment History"):
		employment_fields = _existing_fields(
	...
	if _doctype_exists("SH Baseline"):
		baseline_fields = _existing_fields(
			"SH Baseline", ["name", "milestone", "date_submitted", "programme_schedule", "class"]
		)
	...
	return {
		"student": {
			...
			"date_of_birth": str(student_doc.get("date_of_birth"))
			if student_doc.get("date_of_birth")
			else None,
			...
			"user_login_email": _pick_first(
				student_doc, ["user_login_email", "pestalozzi_student_email", "personal_email"]
			),
			"guardian_name": student_doc.get("guardian_name"),
			"guardian_mobile_number": student_doc.get("guardian_mobile_number"),
			"enrolment_date": str(student_doc.get("enrolment_date"))
			if student_doc.get("enrolment_date")
			else None,
			...
			"motivations": [
				{"name": m.name, "motivation": m.motivation} for m in (student_doc.get("motivations") or [])
			],
			"resilience_links": [
				{"name": r.name, "resilience_statement": r.resilience_statement}
				for r in (student_doc.get("resilience_links") or [])
			],
...
def mark_attendance(schedule, date, attendance_records):
	...
	existing = frappe.db.exists("SH Attendance", {"sh_programme_schedule": schedule, "date": date})
	if existing:
		frappe.throw(
			_("An attendance session already exists for schedule {0} on {1}.").format(schedule, date)
		)
