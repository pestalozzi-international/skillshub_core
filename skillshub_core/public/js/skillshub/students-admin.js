(function () {
  ...
  function loadData() {
    Promise.allSettled([
      ...
      sf('/api/resource/SH Student?fields=["name","student_name","current_cohort","skillshub_programme","programme_path","status"]&limit=1000').then(function (d) { if (d) allStudents = d.data || []; }),
      sf('/api/resource/SH Student Enrolment?fields=["student","milestone","feedback_submitted","attendance_rate","status"]&limit=5000').then(function (d) { if (d) allEnrolments = d.data || []; })
    ]).then(function () {
  ...
  function renderTable(students) {
    ...
    var html = '<div class="table-wrap"><table><thead><tr><th>Student ID</th><th>Full Name</th><th>Cohort</th><th>Programme</th><th>Path</th><th>Attendance</th><th>Feedback</th><th></th></tr></thead><tbody>';
    students.forEach(function (s) {
      ...
      html += '<tr><td><span class="sid">' + s.name + '</span></td><td>' + (s.student_name||'—') + '</td><td>' + (s.current_cohort||'—') + '</td><td>' + (s.skillshub_programme||'—') + '</td><td>' + (s.programme_path||'—') + '</td><td><span class="' + attCls + '">' + avgAtt + '%</span></td><td><span class="' + (fbOk?'fb-yes':'fb-no') + '">' + (fbOk?'✓ Done':'⚠ Pending') + '</span></td><td><a href="/skillshub/admin/student?id=' + encodeURIComponent(s.name) + '" class="view-btn">View</a></td></tr>';
    });
