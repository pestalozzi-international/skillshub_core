(function () {
  ...
  function renderForm(settings) {
    ...
    form.innerHTML = FIELDS.map(function (field) {
      ...
      if (field.type === 'textarea') {
        control = '<textarea class="sh-input" rows="3" data-key="' + esc(field.key) + '">' + esc(value) + '</textarea>';
      } else {
        control = '<input class="sh-input" data-key="' + esc(field.key) + '" type="' + esc(field.type) + '" value="' + esc(value) + '">';
      }
      return '<div class="sh-input-group"><label class="sh-label">' + esc(field.label) + '</label>' + control + '</div>';
    }).join('');
  }
  ...
  function loadSettings() {
    setMessage('');
    api('/api/method/skillshub_core.skillshub_portal.doctype.skillshub_portal_settings.skillshub_portal_settings.get_portal_settings')
      .then(function (settings) {
        renderForm(settings || {});
      })
      .catch(function (error) {
        setMessage('<div class="sh-alert-error">Unable to load settings: ' + esc(error.message) + '</div>');
      });
  }
  ...
  function saveSettings() {
    ...
    api('/api/method/skillshub_core.skillshub_portal.api.update_portal_settings', {
      ...
      .catch(function (error) {
        setMessage('<div class="sh-alert-error">Save failed: ' + esc(error.message) + '</div>');
      })
      .finally(function () {
