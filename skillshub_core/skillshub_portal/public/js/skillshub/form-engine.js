(function () {
	...
	function resolveBackLink() {
		...
		if (student) {
			var isAdmin = !!(
				window.SHPortal &&
				window.SHPortal.bootstrap &&
				window.SHPortal.bootstrap.is_admin
			);
			var referrer = document.referrer || "";
			if (
				isAdmin ||
				referrer.indexOf("/skillshub/admin/student") > -1 ||
				referrer.indexOf("/skillshub/admin/") > -1
			) {
				backLink.href = "/skillshub/admin/student?id=" + encodeURIComponent(student);
				return;
			}
		...
		for (var index = 0; index < names.length; index += 1) {
			if (
				fields.some(function (field) {
					return field.fieldname === names[index];
				})
			)
				return names[index];
		}
	...
	function inferEnrolmentField() {
		...
		return fields.some(function (field) {
			return field.fieldname === "enrolment_ticket";
		})
			? "enrolment_ticket"
			: null;
	}
	...
	function chosenEnrolment() {
		...
		if (ticket) {
			var direct = enrolments.find(function (row) {
				return row.name === ticket;
			});
			if (direct) return direct;
		}
		if (schedule) {
			var byClass = enrolments.find(function (row) {
				return row.class === schedule;
			});
	...
	function contextDefaults() {
		...
		var schedule =
			...
			summaryStudent.current_schedule ||
			"";
		var ticket =
			params.get("enrolment_ticket") ||
			(enrolment && enrolment.name) ||
			summaryStudent.current_enrolment ||
			"";
	...
	function setContextCard() {
		...
		var html =
			"" +
			'<div class="kv-item"><strong>Student</strong><div>' +
			esc(summaryStudent.student_name || data.student || "—") +
			"</div></div>" +
			'<div class="kv-item"><strong>Student ID</strong><div>' +
			esc(data.student || "—") +
			"</div></div>" +
			'<div class="kv-item"><strong>Class</strong><div>' +
			esc(data.schedule || "—") +
			"</div></div>" +
			'<div class="kv-item"><strong>Enrolment Ticket</strong><div>' +
			esc(data.enrolment_ticket || "—") +
			"</div></div>" +
			'<div class="kv-item"><strong>Programme Path</strong><div>' +
			esc(data.programme_path || "—") +
			"</div></div>" +
			'<div class="kv-item"><strong>Course Run</strong><div>' +
			esc(data.course_run || "—") +
			"</div></div>";
		card.innerHTML = html;
	}
	...
	function setFieldContext(field) {
		...
		if (field.fieldname === "course" || field.fieldname === "skillshub_course")
			return defaults.course;
		if (field.fieldname === "cohort" || field.fieldname === "skillshub_cohort")
			return defaults.cohort;
	...
	function renderLinkField(field, value) {
		return getLinkOptions(field.options).then(function (options) {
			...
			return (
				'<select class="sh-input" data-fieldname="' +
				esc(field.fieldname) +
				'" data-fieldtype="Link">' +
				html +
				"</select>"
	...
	function renderSimpleField(field, value) {
		...
		if (field.fieldtype === "Check") {
			return Promise.resolve(
				'<input type="checkbox" class="sh-input" data-fieldname="' +
					fieldname +
		...
		if (field.fieldtype === "Select") {
			var select =
				'<select class="sh-input" data-fieldname="' +
				fieldname +
				'" data-fieldtype="Select"><option value=""></option>';
			parseSelectOptions(field.options).forEach(function (option) {
				select +=
					'<option value="' +
					esc(option) +
					'"' +
					(String(current) === option ? " selected" : "") +
					">" +
					esc(option) +
					"</option>";
			});
		...
		if (field.fieldtype === "Date") {
			return Promise.resolve(
				'<input type="date" class="sh-input" data-fieldname="' +
					fieldname +
		...
		if (field.fieldtype === "Datetime") {
			return Promise.resolve(
				'<input type="datetime-local" class="sh-input" data-fieldname="' +
					fieldname +
		...
		if (
			...
			return Promise.resolve(
				'<input type="number" class="sh-input" data-fieldname="' +
					fieldname +
					'" data-fieldtype="' +
					esc(field.fieldtype) +
					'" value="' +
		...
		if (field.fieldtype === "Rating") {
			return Promise.resolve(
				'<input type="range" min="1" max="5" class="sh-input" data-fieldname="' +
					fieldname +
		...
		if (
			...
			return Promise.resolve(
				'<textarea class="sh-input" rows="4" data-fieldname="' +
					fieldname +
					'" data-fieldtype="' +
					esc(field.fieldtype) +
					'">' +
		...
		return Promise.resolve(
			'<input type="text" class="sh-input" data-fieldname="' +
				fieldname +
				'" data-fieldtype="' +
				esc(field.fieldtype) +
				'" value="' +
	...
	function tableValueField(meta) {
		...
		var preferred = meta.fields.find(function (field) {
			return field.fieldtype === "Link";
		});
		if (preferred) return preferred;
		preferred = meta.fields.find(function (field) {
			return field.fieldtype === "Select";
		});
		if (preferred) return preferred;
		preferred = meta.fields.find(function (field) {
			return ["Data", "Small Text", "Text"].indexOf(field.fieldtype) > -1;
		});
	...
	function renderTableField(field, meta, value) {
		...
		var selected = (value || [])
			.map(function (row) {
				return row[rowField.fieldname];
			})
			.filter(Boolean);
		var promise = Promise.resolve([]);
		if (rowField.fieldtype === "Link") promise = getLinkOptions(rowField.options);
		if (rowField.fieldtype === "Select")
			promise = Promise.resolve(parseSelectOptions(rowField.options));

		return promise.then(function (options) {
			if (!options.length)
				return (
					'<div style="font-size:0.8rem;color:var(--muted-text-color);">No options configured for ' +
					esc(rowField.label || rowField.fieldname) +
					".</div>"
				);
			var html =
				'<div class="table-matrix" data-fieldname="' +
				esc(field.fieldname) +
				'" data-row-field="' +
				esc(rowField.fieldname) +
				'">';
			options.forEach(function (option) {
	...
	function renderCreateForm(existingDoc) {
		...
		var renderPromises = fields.map(function (field) {
			...
			var initialValue =
				contextualValue !== null && contextualValue !== undefined && contextualValue !== ""
					? contextualValue
					: existingValue;
			...
			if (field.read_only && !existingDoc) {
				return Promise.resolve(
					'<div class="sh-input-group"><label class="sh-label">' +
						...
						"</label>" +
						'<input class="sh-input" data-fieldname="' +
						esc(field.fieldname) +
						'" data-fieldtype="' +
						esc(field.fieldtype) +
						'" value="' +
			...
			if (field.fieldtype === "Table") {
				var childMeta = state.meta.child_tables
					? state.meta.child_tables[field.fieldname]
					: null;
	...
	function gatherPayload() {
		var payload = {};
		document
			.querySelectorAll("#portal-form-body [data-fieldname][data-fieldtype]")
			.forEach(function (input) {
				...
				if (fieldtype === "Check") payload[fieldname] = input.checked ? 1 : 0;
					...
					payload[fieldname] = input.value === "" ? null : Number(input.value);
				else payload[fieldname] = input.value;
			});

		document
			.querySelectorAll("#portal-form-body .table-matrix[data-fieldname][data-row-field]")
		...
		api("/api/method/skillshub_core.skillshub_portal.api.submit_portal_form", {
			...
			.then(function (result) {
				...
				document.getElementById("portal-form-message").innerHTML =
					'<div class="sh-alert-success">Submitted successfully. Reference: <strong>' +
					esc(result.name) +
					"</strong></div>";
			})
			.catch(function (error) {
				...
				document.getElementById("portal-form-message").innerHTML =
					'<div class="sh-alert-error">Submission failed: ' +
					esc(error.message) +
					"</div>";
			});
	}
	...
	function bootstrap() {
		...
		Promise.all([
			...
			.catch(function (error) {
				document.getElementById("portal-form-body").innerHTML =
					'<div class="sh-alert-error">Unable to load form: ' +
					esc(error.message) +
					"</div>";
			});
	}
