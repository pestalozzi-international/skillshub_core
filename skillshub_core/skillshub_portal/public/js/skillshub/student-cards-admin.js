...
(function () {
	...
	function buildCard(s) {
		...
		return (
			...
			'<div class="pi-student-avatar">' +
			avatar +
			"</div>" +
			'<div style="flex:1;min-width:0;">' +
			'<p class="pi-card-name" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' +
			esc(s.student_name || s.name) +
			"</p>" +
			'<div style="display:flex;align-items:center;gap:0.35rem;flex-wrap:wrap;">' +
			'<p class="pi-card-id">' +
			esc(s.name) +
			"</p>" +
			'<button class="pi-copy-btn" data-copy="' +
			esc(s.name) +
			'" title="Copy ID">Copy</button>' +
			"</div>" +
			"</div>" +
			"</div>" +
			dobRow +
			...
			'<a href="/skillshub/admin/student?id=' +
			encodeURIComponent(s.name) +
			'" class="pi-btn pi-btn-secondary pi-btn-sm" style="flex:1;text-decoration:none;text-align:center;">View Profile</a>' +
			"</div>" +
			"</div>"
		);
	}
	...
	function renderCards(items) {
		...
		if (!items || !items.length) {
			grid.innerHTML =
				'<div style="grid-column:1/-1;text-align:center;padding:3rem;color:var(--pi-muted);">No students found.</div>';
			return;
		}
	...
	function renderPagination() {
		...
		var html =
			'<button class="pi-pager" id="pi-prev-btn"' +
			(current <= 1 ? " disabled" : "") +
			">‹</button>";
		...
		html +=
			'<button class="pi-pager" id="pi-next-btn"' +
			(current >= pages ? " disabled" : "") +
			">›</button>";
		html +=
			'<span class="pi-text-xs pi-text-muted" style="margin-left:0.5rem;">' +
			total +
			" students</span>";
		bar.innerHTML = html;
	}
	...
	function loadCards() {
		setLoading(true);
		var qs =
			"/api/method/skillshub_core.skillshub_portal.api.get_student_cards?" +
			"page=" +
			state.page +
			"&page_size=" +
			state.pageSize +
		...
		api(qs)
			...
			.catch(function (err) {
				...
				if (grid)
					grid.innerHTML =
						'<div class="pi-alert pi-alert-error" style="grid-column:1/-1;">' +
						esc(err.message) +
						"</div>";
			});
	}
	...
	function init() {
		...
		["pi-filter-status", "pi-filter-path", "pi-filter-cohort", "pi-filter-size"].forEach(
			function (id) {
				var el = document.getElementById(id);
				if (!el) return;
				el.addEventListener("change", function () {
					if (id === "pi-filter-size") {
						state.pageSize = parseInt(el.value) || 24;
						state.page = 1;
					}
					applyFilters();
				});
		...
		document.addEventListener("click", function (e) {
			...
			if (pager) {
				state.page = parseInt(pager.getAttribute("data-page"));
				loadCards();
				return;
			}
