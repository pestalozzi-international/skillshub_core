...
(function () {
  ...
  function api(path, opts) {
    ...
    return fetch(path, Object.assign({ credentials: 'include', headers: headers }, opts || {}))
      .then(function (r) {
        return r.json().then(function (d) {
          if (!r.ok) {
            var msg = (d && (d._server_messages || d.message || d.exc_type)) || ('HTTP ' + r.status);
            if (d && d._server_messages) {
              try { msg = JSON.parse(d._server_messages)[0]; } catch (e) {}
  ...
  function buildOverlay(formTitle) {
    ...
    div.innerHTML =
      '<div class="pi-gate-card">' +
        ...
        '<h2 class="pi-gate-title">' + esc(formTitle || 'SkillsHub Portal') + '</h2>' +
        ...
        '<div class="pi-gate-form">' +
          ...
          '<div class="pi-field">' +
            ...
            '<input id="sh-gate-dob" class="pi-input" type="date" max="' + new Date().toISOString().split('T')[0] + '">' +
          '</div>' +
  ...
  function handleVerify() {
    ...
    showGateError('');
    if (!sid) { showGateError('Please enter your Student ID.'); if (sidEl) sidEl.focus(); return; }
    if (!dob) { showGateError('Please enter your Date of Birth.'); if (dobEl) dobEl.focus(); return; }
    ...
    api('/api/method/skillshub_core.skillshub_portal.api.verify_student_public', {
      ...
      .then(function (result) {
        ...
        dismissOverlay();
        fire({ token: result.token, sid: result.student_id, name: result.student_name, path: result.programme_path });
      })
      .catch(function (err) {
