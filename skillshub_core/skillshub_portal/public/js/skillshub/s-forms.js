...
(function () {
	...
	var SECTION_META = {
		assessment: {
			...
			icon: "📋",
			hint: "These can be submitted multiple times.",
		},
	...
	function buildStudentCard(student, enrolment) {
		var cohort = student.intake_cohort
			? '<span class="pi-badge pi-badge-sky" style="margin-left:0.5rem;">' +
			  esc(student.intake_cohort) +
			  "</span>"
			: "";
		var path = student.programme_path
			? '<span class="pi-badge pi-badge-red" style="margin-left:0.25rem;">Path ' +
			  esc(student.programme_path.replace("Path ", "")) +
			  "</span>"
			: "";
		var status = student.status
			? '<span class="pi-badge pi-badge-sand">' + esc(student.status) + "</span>"
			: "";
		var enrolInfo = enrolment
			? '<div class="pi-text-xs pi-text-muted" style="margin-top:0.25rem;">Current class: <strong>' +
			  esc(enrolment["class"] || enrolment.course || "N/A") +
			  "</strong></div>"
		...
		return (
			...
			'<div class="pi-student-avatar" style="width:3.5rem;height:3.5rem;font-size:1.2rem;">' +
			esc(initials(student.student_name || student.name)) +
			"</div>" +
			'<div style="flex:1;">' +
			'<div style="font-size:1.1rem;font-weight:800;">' +
			esc(student.student_name || student.name) +
			"</div>" +
			'<div style="font-size:0.82rem;color:var(--pi-muted);font-family:monospace;">' +
			esc(student.name) +
			"</div>" +
			...
			enrolInfo +
			"</div>" +
			'<a href="/skillshub/s/profile?token=' +
			encodeURIComponent(state.session.token) +
			'" class="pi-btn pi-btn-ghost pi-btn-sm">Edit Profile</a>' +
			"</div>" +
			"</div>"
		);
	}
	...
	function buildFormItem(form) {
		...
		var badge = done
			...
			: multi
			? '<span class="pi-badge pi-badge-sky">Multiple allowed</span>'
			: '<span class="pi-badge pi-badge-sand">Once per enrolment</span>';

		var actionBtn = done
			? '<span class="pi-badge pi-badge-green" style="padding:0.45rem 1rem;">✓ Done</span>'
			: '<a href="' +
			  esc(form.route) +
			  '" class="pi-btn pi-btn-primary pi-btn-sm" style="white-space:nowrap;">' +
			  (multi ? "+ Submit New" : "Submit →") +
			  "</a>";
		...
		return (
			...
			(done ? " done" : "") +
			'">' +
			'<div class="pi-form-item-icon">' +
			icon +
			"</div>" +
			'<div class="pi-form-item-info">' +
			'<div class="pi-form-item-name">' +
			esc(form.label) +
			"</div>" +
			'<div class="pi-form-item-hint">' +
			badge +
			"</div>" +
			"</div>" +
			"<div>" +
	...
	function renderHub() {
		...
		Object.keys(sections).forEach(function (key) {
			...
			html +=
				'<div class="pi-section-label">' + meta.icon + " " + esc(meta.title) + "</div>";
			if (meta.hint)
				html +=
					'<p class="pi-text-xs pi-text-muted" style="margin:-0.75rem 0 0.75rem;">' +
					esc(meta.hint) +
					"</p>";
			sections[key].forEach(function (f) {
				html += buildFormItem(f);
			});
		});

		if (!ctx.enrolment) {
			html +=
				'<div class="pi-alert pi-alert-info" style="margin-top:1rem;">' +
	...
	function load() {
		...
		if (root)
			root.innerHTML =
				'<div style="text-align:center;padding:2rem;color:var(--pi-muted);">Loading your forms…</div>';
